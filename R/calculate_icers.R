...
plot_strategies <- function(icers, labels=FALSE) {
  if (is.null(icers)) {
    ...
    icers$Status <- factor(icers$Status, levels = c("D", "ED", "ND"),
                          labels = c("Dominated", "Extendedly Dominated", "Non-Dominated"))
    icer_plot <- ggplot(icers, aes(x = Effect, y = Cost, group = Status, colour = Status,
                                   linetype = Status, label = Strategy)) +
