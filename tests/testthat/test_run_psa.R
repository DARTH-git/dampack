...
test_func <- function(params, extra_param){
  ...
  effect1 <- normal_param + lognorm_param * extra_param + beta_param + gamma_param + level1 + level2 + level3 + bootstrap_param
  cost1 <- - normal_param - lognorm_param - beta_param - gamma_param - level1 - level2 - level3 - bootstrap_param

  effect2 <- normal_param - lognorm_param * extra_param + beta_param - gamma_param + level1 - level2 + level3 - bootstrap_param
...
test_that("run_psa runs", {
  expect_silent(runtest<- run_psa(test, test_func, outcomes = c("cost","effect"), cost_outcome = "cost",
                                  ...
                                  strategies = c("customstrat1", "customstrat2"), currency = "$", extra_param = 1.5))
})
