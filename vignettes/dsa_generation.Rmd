...
Deterministic sensitivity analysis (DSA) is a method for assessing how the results of a decision analytic model vary over a pre-specified range of values for a set of parameters of interest. 

In contrast to the probabilistic sensitivity analysis (PSA), where the variation of the parameter of interest is derived from an underlying probability distribution, DSA allows the decision modeler to have exact control over the parameter values to be considered in the analysis. Additionally, in a PSA, all of the other parameters in the model are varying along with the parameter of interest, whereas in the DSA, all of the other parameters are held at their "base case" values.
...
As an example, we define the function `run_decision_model` below, which is a cohort state transition model (also known as a Markov model) that evaluates two strategies ("no treatment" and "treatment") for a hypothetical disease. For a deeper discusison of state transition models and more complex variations of this state transition model, see Alarid-Escudero F, Krijkamp EM, Enns EA, Yang A, Hunink MGM, Pechlivanoglou P, Jalal H. Cohort state-transition models in R: A Tutorial. arXiv:200107824v1. 2020:1-31. `run_decision_model` has as inputs `l_params_all`, which is the input parameter values list. It also has an additional optional argument `wtp` which allows the user to change the willingness-to-pay used in the model when calculating the net monetary benefit model outcome.
