...
`ceac` stands for cost-effectiveness acceptability curve(s), and the `ceac()` function takes a `psa` object and a numeric vector of willingness to pay thresholds to create a `ceac` object, which is essentially a data.frame with special plotting functionality.
